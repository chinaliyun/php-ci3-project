...
class Team extends My_Controller{
	...
	public function index(){
		$data['subpage'] = 'admin/team.html';
		$data['current_nav'] = 'team';
		$data['path'] = '文章管理 / 文章列表';
		...
		$data['team'] = $this->team->check_page($config['per_page'],$this->uri->segment(4));
	...
	public function edit(){
		$data['subpage'] = 'admin/team_edit.html';

		$data['current_nav'] = 'teamEdit';
		$data['path'] = '文章管理 / 发布文章';
		$data['company'] = '上海杰汉申若丝琳装饰有限公司';

		if($this->uri->segment(4, 'null') != 'null'){
			$nid = $this->uri->segment(4);
			$data['team'] = $this->team->check_nid($nid)[0];
		}
		$this->load->view('admin/tpl.html',$data);
	}
	...
	*/
	...
	public function send(){
		...
		if($nid == ''){
			if(!$this->team->add_team($data)){
				echo 0;
			};
			echo 1;
		}else{
			//获取旧的thumb再上传新的,便于删除旧图片
			$team = $this->team->check_nid($nid)[0];
			$oldthumb = 'public/upload/'.$team['thumb'];
			if(!$this->team->upload_team($nid, $data)){
				echo 0;die;
			}
